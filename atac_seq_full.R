...
#genome <- "/lab/solexa_lehmann/Dros-ATAC-seq-analysis/Greenleaf_original_ATAC_data/hg19_Genome.fa"
...
outBAM_postqc <- "/lab/solexa_lehmann/Dros-ATAC-seq-analysis/ATAC.bam"

#Set file path for the post quality control sorted file
sortedBAM_postqc <- "/lab/solexa_lehmann/Dros-ATAC-seq-analysis/Sorted_ATAC.bam"
...
#Only plotting reads that mapped to Chromosome 20
atacReads <- readGAlignmentPairs(sortedBAM_postqc, param = ScanBamParam(mapqFilter = 1, 
    flag = scanBamFlag(isPaired = TRUE, isProperPair = TRUE), what = c("qname", 
        "mapq", "isize")))
...
pdf(file="/lab/solexa_lehmann/Dros-ATAC-seq-analysis/fragment_lengths_0.3xTn5.pdf", width = 5, height = 5)
fragLenPlot + theme_bw() + xlim(0,2000) + ylim(0,3000)
dev.off()

#Fragment length plot with y axis in log scale
pdf(file="/lab/solexa_lehmann/Dros-ATAC-seq-analysis/fragment_lengths_ylog_0.3xTn5.pdf", width = 5, height = 5)
#plot on a log scale
